...
- **内置路由**：内置 V2EX 热帖、B 站 UP 主专栏 / 视频等站点适配器，没有 RSS 也能直接订阅（B 站等反爬站点由浏览器内核渲染抓取）
...
1. **添加订阅源**：点击侧边栏顶部的「+」→ 在「添加订阅源」窗口粘贴博客 / 网站的 RSS 地址（例如 `https://example.com/feed.xml`），或切换到**内置路由**卡片直接选择支持的站点（如 V2EX 热帖、B 站 UP 主视频 / 专栏）→ 确定。应用会自动识别标题和图标。
...
> **B 站等站点需要登录态？** 在「设置 → 内置路由」里可以粘贴对应站点的登录 Cookie（浏览器里登录后复制，如 B 站的 `SESSDATA`）或点「用浏览器登录」，能让抓取更稳定。未配置 Cookie 也能订阅公开内容，只是可能遇到风控、抓取不稳定。
...
> 翻译功能默认启用微软翻译（免费，无需配置）；如需在设置中改用百度翻译，填入 AppID 与密钥后即可，菜单项同样会随状态自动启用或置灰。

## 常见问题

### 我的数据存在哪里？
所有订阅和文章都保存在本地 SQLite 数据库中，数据完全属于你自己。数据库文件位置：
...
### 我订阅的网站没有 RSS 怎么办？
先看看**内置路由**里有没有适配过——目前内置 V2EX 热帖、B 站 UP 主专栏与视频，直接选择即可订阅。若没有对应站点，再考虑用第三方服务（如 [RSSHub](https://docs.rsshub.app/)）为它生成一个订阅地址，然后粘贴进来。
