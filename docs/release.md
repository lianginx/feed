...
4. 推送 tag 即触发 GitHub Actions 构建三平台并创建 draft release；draft 创建后立即用 `gh release edit vX.Y.Z --notes-file <file>` 覆盖 notes（脚本生成的 notes 仅作兜底）
