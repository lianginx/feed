...
当前仅支持并只为 macOS 设计。Windows / Linux 版本已停止发布，不投入适配工作：

- 功能设计与实现只考虑 macOS 的平台特性（如标题栏样式、Dock、菜单栏、全局快捷键习惯等），不需要为其他平台设计对应交互
- 允许顺手保留或编写简单的平台分支（如一行 `process.platform === 'darwin'` 的 if），但不做复杂的多平台适配：不引入平台抽象层、不为其他平台编写专门的实现分支或构建逻辑
- 涉及平台判断时优先以「macOS 为主、其余平台不保证」的立场取舍，避免为未测试的平台增加维护成本
- 不得主动提议恢复 Windows / Linux 支持，除非用户明确提出
