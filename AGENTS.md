...
## 代码规范
- 渲染层 import 路径约定（`@renderer`、`@/` 均指向 `src/renderer/src`）：
  - 业务代码一律使用 `@renderer` 别名，禁止相对路径
  - `@/` 仅限 shadcn 生成的 `components/ui/` 组件内部使用（如 `@/lib/utils`、`@/components/ui/button`），业务代码禁止使用
  - `components/ui/` 组件内部之间互相引用允许使用相对路径
