...
## 发版规范（GitHub Release）
<constraints>
1. 发版前必须先更新 `package.json` 的 `version` 为 `X.Y.Z` 并提交（`chore: 版本号升至 X.Y.Z`），tag 指向的代码必须包含该更新，禁止版本号滞后的发版。
2. 推送 `vX.Y.Z` tag 即触发 GitHub Actions（`.github/workflows/release.yml`）自动构建并发布，全程无需手动干预。
3. 发版流程三步：
